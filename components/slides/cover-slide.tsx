...
const MODULES = [
  { icon: LayoutGrid, label: "Market Overview" },
  { icon: LineChart, label: "Live Chart" },
  { icon: Newspaper, label: "News Research" },
  { icon: TrendingUp, label: "Macro Indicators" },
]
...
/** The actual working Sign In / Sign Up form — shown after the teaser strip is dismissed. */
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        <svg className="w-4 h-4" viewBox="0 0 24 24">
          ...
          <path fill="#FBBC05" d="M5.6 14.8c-.2-.7-.4-1.5-.4-2.3s.2-1.6.4-2.3L1.9 7.3C.7 9.7 0 12 0 14.8s.7 5.1 1.9 7.5l3.7-2.9c-.6-.7-1-1.7-1-2.9z" />
      ...
      <div className="flex items-center gap-2 my-0.5">
        ...
        <span className="text-[10px] text-zinc-600 font-medium uppercase tracking-wider">atau Email</span>
        <div className="flex-1 h-px bg-zinc-800" />
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center gap-5 px-6 text-center max-w-lg w-full">
        ...
        <div
          className={`w-full mt-2 rounded-2xl p-6 transition-all duration-300 ${
            shakeAlert ? "border-red-500/60 shadow-red-500/20" : ""
          }`}
          style={{
        ...
        >
          {isAuthenticated ? (
            ...
            /* Live proof strip — real session status + clock, not market prices
               (those live in Market Overview, showing them here would just spoil it) */
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between gap-2 p-3 rounded-xl bg-zinc-950/60 border border-zinc-800">
                {liveClock ? (
                  <>
                    <div className="flex items-center gap-2">
                      <span className="relative flex h-2 w-2">
                        {isForexMarketOpen(liveClock) && (
                          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
                        )}
                        <span className={`relative inline-flex rounded-full h-2 w-2 ${isForexMarketOpen(liveClock) ? "bg-emerald-400" : "bg-zinc-600"}`} />
                      </span>
                      <span className="text-xs font-semibold text-zinc-200">
                        {isForexMarketOpen(liveClock) ? getActiveSessions(liveClock).join(" · ") || "Market Open" : "Market Closed"}
                      </span>
                    </div>
                    <span className="text-[11px] font-mono text-zinc-500">
                      UTC {liveClock.toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit", second: "2-digit", timeZone: "UTC" })}
                    </span>
                  </>
                ) : (
                  <span className="text-xs text-zinc-600">Memuat status pasar...</span>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {MODULES.map((m) => (
                  <div
                    key={m.label}
                    className="flex items-center gap-2 p-2.5 rounded-xl bg-zinc-900/50 border border-zinc-800"
                  >
                    <m.icon className="h-3.5 w-3.5 text-amber-400/80 shrink-0" />
                    <span className="text-[11px] text-zinc-300 leading-tight">{m.label}</span>
                  </div>
                ))}
              </div>
              ...
              <button
                ...
                className="w-full mt-1 py-2.5 px-4 rounded-xl font-semibold text-xs text-amber-950 bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 transition-all duration-200 shadow-lg shadow-amber-500/15 flex items-center justify-center gap-2"
              >
                <span>Mulai</span>
                <span>→</span>
              </button>
